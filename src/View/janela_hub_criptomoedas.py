...
from src.View.janela_hub_cripto_painel_periodo import JanelaHubCriptoPainelPeriodo
...
class JanelaHubCriptomoedas(ctk.CTkToplevel):
    """Painel dedicado de criptomoedas com as mesmas funcoes do mercado de acoes."""

    def __init__(self, pai: ctk.CTk) -> None:
        super().__init__(pai)
        self._controlador = ControladorCripto()
        ...
        self._janela_cripto_periodo: JanelaHubCriptoPainelPeriodo | None = None
    ...
    def _montar_layout(self) -> None:
        ...
        self._config_hub.montar_titulo_com_engrenagem(cabecalho, "Criptomoedas")

        ctk.CTkLabel(
            cabecalho,
            text="Cotacoes em USD, graficos, favoritos, comparacao e noticias de cripto.",
    ...
    def _montar_consultas(self) -> None:
        ...
        ctk.CTkLabel(
            card,
            text="Pesquise criptos, gerencie favoritos, compare desempenho e leia noticias.",
            font=ctk.CTkFont(size=12),
            text_color=CORES["textoSecundario"],
        ...
        for texto, comando in (
            ("Pesquisar cripto", self._abrir_pesquisa),
            ("Favoritas", self._abrir_favoritas),
            ("Comparar", self._abrir_comparar),
            ("Noticias", self._abrir_noticias),
            ("Criptomoedas por periodo", self._abrir_criptos_por_periodo),
        ):
            ctk.CTkButton(
                linha,
                text=texto,
        ...
        qtd = self._config_painel.carregar_quantidade_cripto()
        self._label_status = ctk.CTkLabel(
            barra,
            text=f"Carregando ate {qtd} criptos em cada aba...",
            font=ctk.CTkFont(size=12),
            text_color=CORES["textoSecundario"],
        )
    ...
    def _montar_painel_cotacoes(self) -> None:
        ...
        self._tabela_alta = self._criar_tabela_aba(
            self._aba_alta, "Criptos em alta no dia (duplo clique abre o grafico)"
        )
        self._tabela_queda = self._criar_tabela_aba(
            self._aba_queda, "Criptos em queda no dia (duplo clique abre o grafico)"
        )
        self._tabela_todas = self._criar_tabela_aba(
            self._aba_todas, "Todas as criptos monitoradas (duplo clique abre o grafico)"
        )
    ...
    def _abrir_pesquisa(self) -> None:
        if self._focar_janela(self._janela_pesquisa):
            return
        self._janela_pesquisa = JanelaPesquisaAcao(
            self, self._controlador, modo_cripto=True
        )
    ...
    def _abrir_favoritas(self) -> None:
        if self._focar_janela(self._janela_favoritas):
            ...
            return
        self._janela_favoritas = JanelaFavoritas(self, self._controlador, modo_cripto=True)
    ...
    def _abrir_noticias(self) -> None:
        if self._focar_janela(self._janela_noticias):
            return
        self._janela_noticias = JanelaNoticiasMercado(
            self, self._controlador, modo_cripto=True
        )

    def _abrir_criptos_por_periodo(self) -> None:
        if self._focar_janela(self._janela_cripto_periodo):
            return
        self._janela_cripto_periodo = JanelaHubCriptoPainelPeriodo(self)

"""Hub de criptomoedas com painel em alta / queda / todas filtrado por periodo."""
...
class JanelaHubCriptoPainelPeriodo(ctk.CTkToplevel):
    """Mesma estrutura de FIIs por periodo, para criptomoedas."""
    ...
    def __init__(self, pai: ctk.CTk) -> None:
        ...
        self.title("Criptomoedas por periodo")
    ...
    def _montar_layout(self) -> None:
        ...
        self._config_hub.montar_titulo_com_engrenagem(cabecalho, "Criptomoedas por periodo")

        ctk.CTkLabel(
            cabecalho,
            text=(
                "Em alta, em queda e todas as criptomoedas monitoradas com variacao "
    ...
    def _montar_consultas(self) -> None:
        ...
        self._label_status = ctk.CTkLabel(
            barra,
            text=f"Carregando ate {qtd} criptos no periodo Mes...",
            font=ctk.CTkFont(size=12),
            text_color=CORES["textoSecundario"],
        )
    ...
    def _titulo_aba(self, tipo: str) -> str:
        ...
        textos = {
            "alta": f"Criptos em alta no periodo: {periodo}",
            "queda": f"Criptos em queda no periodo: {periodo}",
            "todas": f"Todas as criptos no periodo: {periodo}",
        }
        return textos.get(tipo, f"Criptos — {periodo}") + " (duplo clique abre o grafico)"
    ...
    def _carregar_painel(self, automatico: bool = False) -> None:
        ...
        self._label_status.configure(
            text=f"Calculando variacao no periodo {rotulo_periodo} para ate {quantidade} criptos..."
        )
        ...
        def ao_concluir(resultado, erro_thread):
            ...
            if msg_erro:
                ...
                if not automatico:
                    messagebox.showwarning("Criptomoedas por periodo", msg_erro, parent=self)
                return
            ...
            self._preencher(
                self._tabela_alta,
                dados["em_alta"],
                f"Nenhuma cripto em alta no periodo {dados['periodo_rotulo']}.",
            )
            self._preencher(
                self._tabela_queda,
                dados["em_queda"],
                f"Nenhuma cripto em queda no periodo {dados['periodo_rotulo']}.",
            )

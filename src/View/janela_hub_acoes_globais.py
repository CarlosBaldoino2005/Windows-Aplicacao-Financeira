"""Hub de acoes globais (BDRs negociados na B3)."""
...
from src.Controller.controlador_acoes_globais import ControladorAcoesGlobais
...
from src.View.janela_hub_acoes_globais_painel_periodo import JanelaHubAcoesGlobaisPainelPeriodo
...
class JanelaHubAcoesGlobais(ctk.CTkToplevel):
    """Painel dedicado a BDRs (acoes globais negociadas na B3)."""

    def __init__(self, pai: ctk.CTk) -> None:
        super().__init__(pai)
        self._controlador = ControladorAcoesGlobais()
        ...
        self._janela_periodo: JanelaHubAcoesGlobaisPainelPeriodo | None = None
        ...
        self._config_hub = ConfiguracaoHubPainel(
            self,
            self._config_painel,
            escopo_quantidade="acoes_globais",
    ...
    def _montar_layout(self) -> None:
        ...
        self._config_hub.montar_titulo_com_engrenagem(cabecalho, "Acoes globais")

        ctk.CTkLabel(
            cabecalho,
            text=(
                "BDRs de empresas internacionais negociados na B3. Graficos, favoritos, "
                "comparacao e historico em Mais detalhes."
            ),
    ...
    def _montar_consultas(self) -> None:
        ...
        ctk.CTkLabel(
            card,
            text="Pesquise BDRs, favoritos, compare desempenho e leia noticias.",
            font=ctk.CTkFont(size=12),
            text_color=CORES["textoSecundario"],
        ...
        self._label_status = ctk.CTkLabel(
            barra,
            text=f"Carregando ate {qtd} BDRs em cada aba...",
            font=ctk.CTkFont(size=12),
            text_color=CORES["textoSecundario"],
        )
    ...
    def _montar_painel_cotacoes(self) -> None:
        ...
        self._tabela_alta = self._criar_tabela_aba(
            self._aba_alta,
            "BDRs em alta no dia (duplo clique abre o grafico)",
        )
        self._tabela_queda = self._criar_tabela_aba(
            self._aba_queda,
            "BDRs em queda no dia (duplo clique abre o grafico)",
        )
        self._tabela_todas = self._criar_tabela_aba(
            self._aba_todas,
            "Todos os BDRs monitorados (duplo clique abre o grafico)",
        )
    ...
    def _abrir_pesquisa(self) -> None:
        ...
        self._janela_pesquisa.title("Pesquisar BDR")
    ...
    def _abrir_favoritas(self) -> None:
        ...
        self._janela_favoritas = JanelaFavoritas(
            self, self._controlador, tipo_painel="acoes_globais"
        )
        self._janela_favoritas.title("Favoritos — acoes globais")

    def _abrir_comparar(self) -> None:
        if self._focar_janela(self._janela_comparar):
            return
        self._janela_comparar = JanelaCompararAcoes(
            self, self._controlador, modo_somente_acoes_globais=True
        )
        self._janela_comparar.title("Comparar BDRs")
    ...
    def _abrir_noticias(self) -> None:
        ...
        self._janela_noticias.title("Noticias — acoes globais")

    def _abrir_por_periodo(self) -> None:
        if self._focar_janela(self._janela_periodo):
            return
        self._janela_periodo = JanelaHubAcoesGlobaisPainelPeriodo(self)
    ...
    def _carregar_painel(self, automatico: bool = False) -> None:
        ...
        self._label_status.configure(text=f"Carregando ate {quantidade} BDRs...")
        ...
        def ao_concluir(dados, erro):
            ...
            self._preencher(
                self._tabela_alta,
                dados["em_alta"],
                "Nenhum BDR em alta agora.",
            )
            self._preencher(
                self._tabela_queda,
                dados["em_queda"],
                "Nenhum BDR em queda agora.",
            )

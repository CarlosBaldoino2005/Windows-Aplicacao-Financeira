"""Hub de FIIs com painel em alta / queda / todas filtrado por periodo."""
...
class JanelaHubFiisPainelPeriodo(ctk.CTkToplevel):
    """Mesma estrutura de Acoes por periodo, para fundos imobiliarios."""
    ...
    def _montar_layout(self) -> None:
        ...
        self._config_hub.montar_titulo_com_engrenagem(cabecalho, "FIIs por periodo")

        ctk.CTkLabel(
            cabecalho,
            text=(
                "Em alta, em queda e todos os fundos imobiliarios monitorados com variacao "
    ...
    def _montar_consultas(self) -> None:
        ...
        self._label_status = ctk.CTkLabel(
            barra,
            text=f"Carregando ate {qtd} FIIs no periodo Mes...",
            font=ctk.CTkFont(size=12),
            text_color=CORES["textoSecundario"],
        )
    ...
    def _titulo_aba(self, tipo: str) -> str:
        ...
        textos = {
            "alta": f"FIIs em alta no periodo: {periodo}",
            "queda": f"FIIs em queda no periodo: {periodo}",
            "todas": f"Todos os FIIs no periodo: {periodo}",
        }
        return textos.get(tipo, f"FIIs — {periodo}") + " (duplo clique abre o grafico)"
    ...
    def _carregar_painel(self, automatico: bool = False) -> None:
        ...
        self._label_status.configure(
            text=f"Calculando variacao no periodo {rotulo_periodo} para ate {quantidade} FIIs..."
        )
        ...
        def ao_concluir(resultado, erro_thread):
            ...
            if msg_erro:
                ...
                if not automatico:
                    messagebox.showwarning("FIIs por periodo", msg_erro, parent=self)
                return
            ...
            self._preencher(
                self._tabela_alta,
                dados["em_alta"],
                f"Nenhum FII em alta no periodo {dados['periodo_rotulo']}.",
            )
            self._preencher(
                self._tabela_queda,
                dados["em_queda"],
                f"Nenhum FII em queda no periodo {dados['periodo_rotulo']}.",
            )

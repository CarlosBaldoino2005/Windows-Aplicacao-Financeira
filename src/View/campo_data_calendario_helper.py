...
class CampoDataCalendario:
    ...
    def _montar_calendario(self, janela: ctk.CTkToplevel) -> None:
        ...
        for indice, nome in enumerate(["Seg", "Ter", "Qua", "Qui", "Sex", "Sab", "Dom"]):
            ctk.CTkLabel(
                grade,
                text=nome,
                width=34,
        ...
        semanas = calendar.monthcalendar(self._mes_visivel.year, self._mes_visivel.month)
        hoje = date.today()
        for linha, semana in enumerate(semanas, start=1):
            for coluna, dia in enumerate(semana):
                ...
                ctk.CTkButton(
                    ...
                    width=34,
                    height=30,
                    fg_color=CORES["primaria"] if eh_hoje else CORES["borda"],
                    hover_color=CORES["primariaHover"],
                    text_color=CORES.get("textoInverso", "#FFFFFF") if eh_hoje else CORES["texto"],

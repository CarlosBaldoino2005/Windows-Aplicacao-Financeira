"""Hub de BDRs com painel em alta / queda / todas filtrado por periodo."""
...
class JanelaHubAcoesGlobaisPainelPeriodo(ctk.CTkToplevel):
    """Mesma estrutura de Acoes por periodo, para BDRs."""
    ...
    def __init__(self, pai: ctk.CTk) -> None:
        ...
        self.title("Acoes globais por periodo")
    ...
    def _montar_layout(self) -> None:
        ...
        self._config_hub.montar_titulo_com_engrenagem(cabecalho, "Acoes globais por periodo")

        ctk.CTkLabel(
            cabecalho,
            text=(
                "Em alta, em queda e todos os BDRs monitorados com variacao "
    ...
    def _montar_consultas(self) -> None:
        ...
        self._label_status = ctk.CTkLabel(
            barra,
            text=f"Carregando ate {qtd} BDRs no periodo Mes...",
            font=ctk.CTkFont(size=12),
            text_color=CORES["textoSecundario"],
        )
    ...
    def _titulo_aba(self, tipo: str) -> str:
        ...
        textos = {
            "alta": f"BDRs em alta no periodo: {periodo}",
            "queda": f"BDRs em queda no periodo: {periodo}",
            "todas": f"Todos os BDRs no periodo: {periodo}",
        }
        return textos.get(tipo, f"BDRs — {periodo}") + " (duplo clique abre o grafico)"
    ...
    def _carregar_painel(self, automatico: bool = False) -> None:
        ...
        self._label_status.configure(
            text=f"Calculando variacao no periodo {rotulo_periodo} para ate {quantidade} BDRs..."
        )
        ...
        def ao_concluir(resultado, erro_thread):
            ...
            if msg_erro:
                ...
                if not automatico:
                    messagebox.showwarning("Acoes globais por periodo", msg_erro, parent=self)
                return
            ...
            self._preencher(
                self._tabela_alta,
                dados["em_alta"],
                f"Nenhum BDR em alta no periodo {dados['periodo_rotulo']}.",
            )
            self._preencher(
                self._tabela_queda,
                dados["em_queda"],
                f"Nenhum BDR em queda no periodo {dados['periodo_rotulo']}.",
            )

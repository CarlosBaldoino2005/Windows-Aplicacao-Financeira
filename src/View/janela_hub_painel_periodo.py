...
class JanelaHubPainelPeriodo(ctk.CTkToplevel):
    """Mesma estrutura do painel principal, com variacao no periodo escolhido."""
    ...
    def _montar_consultas(self) -> None:
        ...
        self._label_status = ctk.CTkLabel(
            barra,
            text=f"Carregando ate {qtd} acoes no periodo Mes...",
            font=ctk.CTkFont(size=12),
            text_color=CORES["textoSecundario"],
        )
    ...
    def _titulo_aba(self, tipo: str) -> str:
        ...
        textos = {
            "alta": f"Acoes em alta no periodo: {periodo}",
            "queda": f"Acoes em queda no periodo: {periodo}",
            "todas": f"Todas as acoes no periodo: {periodo}",
        }
        return textos.get(tipo, f"Acoes — {periodo}") + " (duplo clique abre o grafico)"
    ...
    def _carregar_painel(self) -> None:
        ...
        def ao_concluir(resultado, erro_thread):
            ...
            self._preencher(
                self._tabela_alta,
                dados["em_alta"],
                f"Nenhuma acao em alta no periodo {dados['periodo_rotulo']}.",
            )
            self._preencher(
                self._tabela_queda,
                dados["em_queda"],
                f"Nenhuma acao em queda no periodo {dados['periodo_rotulo']}.",
            )

"""Painel de BDRs — acoes globais negociadas na B3."""
from datetime import datetime

from src.Model.acoes_bdr_universo import LIMITE_BDRS_PAINEL, montar_lista_bdrs_monitorados
...
class MercadoAcoesGlobaisServico:
    """Cotacoes e historico apenas para BDRs da B3."""

    def __init__(self) -> None:
        self._mercado = MercadoServico()

    def listar_monitoradas(self, quantidade: int = LIMITE_BDRS_PAINEL) -> list[str]:
        return montar_lista_bdrs_monitorados(quantidade)

    def buscar_resumos(self, simbolos: list[str]) -> list[CotacaoResumo]:
        if not simbolos:
            return []
        resumos = self._mercado.buscar_resumos(simbolos)
        return [r for r in resumos if eh_bdr_b3(r.simbolo)]

    def listar_em_alta(self, quantidade: int = LIMITE_BDRS_PAINEL) -> list[CotacaoResumo]:
    ...
    def listar_em_queda(self, quantidade: int = LIMITE_BDRS_PAINEL) -> list[CotacaoResumo]:
        resumos = self.buscar_resumos(self.listar_monitoradas(quantidade))
    ...
    def listar_todas_monitoradas(self, quantidade: int = LIMITE_BDRS_PAINEL) -> list[CotacaoResumo]:
        resumos = self.buscar_resumos(self.listar_monitoradas(quantidade))
    ...
    def comparar_acoes(
        ...
        bdrs = [s for s in simbolos if eh_bdr_b3(s)]
        return self._mercado.comparar_acoes(bdrs, periodo_chave, data_inicio, data_fim)
